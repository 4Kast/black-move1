...
const ItemCart = ({ item }) => {
  ...
  const aumentarQuantidade = () => {
    ...
    dispatch(alertVisibility(`Item Added: ${item.nome}`, 'ok'));
  }

  const diminuirQuantidade = () => {
    dispatch(cartActions.removeOneItemToCart(item.id))
    dispatch(alertVisibility(`Item Removed: ${item.nome}`, 'bad'))
  }
    ...
    <li key={item.id} className={classes.itemCarrinho}>
      ...
      <div className={classes.header}>
        <div className={classes.tituloEPreco}>
          ...
          <p className={classes.unidade}>R {fixedPrecoItem(item.preco)}/Each</p>
        </div>
